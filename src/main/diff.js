...
Diff.prototype.generateDiffParts = function () {
  if (this.remainingExpected.length === 0 && this.remainingActual.length === 0)
    return;
  this.addNextPrefix();
  this.addNextDiff();
  this.generateDiffParts();
...
Diff.prototype.addNextPrefix = function () {
  ...
  const commonPrefix = this.remainingActual
    .slice(0, indexOfNextDiff)
    .join(this.delimiter);
...
Diff.prototype.findIndexOfNextDiff = function () {
  let minLenght = Math.min(
    this.remainingExpected.length,
    this.remainingActual.length
  );
  let i = 0;
  for (; i < minLenght; i++) {
...
Diff.prototype.addNextDiff = function () {
  let { expectedNextCommonIndex, actualNextCommonIndex } =
    this.findIndexOfNextCommon();

  const diffOfExpected = this.remainingExpected.slice(
    0,
    expectedNextCommonIndex
  );
...
Diff.prototype.findIndexOfNextCommon = function () {
  ...
  let maxOfStringLength = Math.max(
    this.remainingExpected.length,
    this.remainingActual.length
  );
  ...
  for (
    ...
    if (actualFirstOccuranceOfItem[remainingExpectedCharAti] !== undefined) {
      actualNextCommonIndex = this.remainingActual.indexOf(
        remainingExpectedCharAti
      );
      b = true;
    }
    if (expectedFirstOccuranceOfItem[remainingActualCharAti] !== undefined) {
      expectedNextCommonIndex = this.remainingExpected.indexOf(
...
Diff.prototype.sliceRemaining = function (
  remainingSliceIndex,
  actualSliceIndex
) {
