...
describe('Verify diff by word', () =>{
    test('Last word in string has diff', () =>{
        ...
        expect(diff.diffString).toBe('words but some are [misspelled|misselled]')
    })
    ...
    test('First word in string has diff', () => {
        ...
        const diff = new WordDiff(s1, s2)
        expect(diff.diffString).toBe('[words|word] but some are misspelled')
    })
    ...
    test('Middle word in string has diff', () => {
        ...
        const diff = new WordDiff(s1, s2)
        expect(diff.diffString).toBe('words but [soe|some] are misspelled')
    })
})